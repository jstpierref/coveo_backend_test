...
class TrieNode:
    """Base object for building the Trie data structure
    
    Attributes:
        children (dict): hash map to store current TrieNode children, if any
...
class Trie:
    ...
    def find_substring_node(self, substring):
        ...
        Returns: 
            TrieNode if any match, None if it does not exist.
        """
    ...
    def add_word(self, reference_id, word, word_type=None, word_position=0):
        ...
        for char in word:
            ...
            current_node = current_node.get_children()[char]
        current_node.add_payload((reference_id, word, word_type, word_position))
    ...
    def autocomplete(self, substring):
        """Interface for searching the trie
    ...
    def add_words(self, content):
        """Interfarce for populating the trie

        Iteratively adds many words and their reference ids to the trie

        Args:
            content (list): list of tuples with words and reference ids
        """
        for reference_id, word, word_type, word_position in content:
            self.add_word(reference_id, word, word_type, word_position)
